...
$(function(){
    ...
    sendForm = function(url, form, redirect){
        ...
        if(url === "/api/web/client/customers/import" || url === "/api/web/client/providers/import" ||
            url === "/api/web/client/partners/import" || url === "/api/web/client/employees/import"){

            if(url === "/api/web/client/customers/import"){ var type_import = 'customers'; }
            if(url === "/api/web/client/providers/import"){ var type_import = 'providers'; }
            if(url === "/api/web/client/partners/import"){ var type_import = 'partners'; }
            if(url === "/api/web/client/employees/import"){ var type_import = 'employees'; }
            ...
            if(url === "/api/web/custom/conciliations/import"){
                ...
                if(url === "/api/web/client/notes/add" || url === "/api/web/client/extracts/add" || 
                    url === "/api/web/client/documents/add" || url === "/api/web/client/documents/add-action" ||
...
$(document).ready(function() {
    ...
    $("#input_customer_type_update, #input_employee_type_update, #input_partner_type_update, #input_provider_type_update").change(function(){
        var input_type = $(this).data('type');
...
addItemDoc = function(){
    ...
    html_itens += '<input type="file" style="display: none;" id="file-document-'+ itens_job +'" class="form-control-file" name="file-document-'+ itens_job +'" onchange="readURL(this, '+itens_job+');">';
    html_itens += '</label>';
...
addItem = function(){
    ...
    html_itens += '<div id="area-add-documents-"'+ itens_job +'">';
    html_itens += '<p class="text margin-t-40 btn-close" style=" margin-bottom: 10px; color: #969696; font-weight: 600; float: left;">Item '+ itens_job +'</p>';
    html_itens += '<button class="text margin-t-40 btn-line-gray" id="remove-'+ itens_job +'"; type="button" style=" margin-bottom: 10px; color: #969696; font-weight: 600; float: right;">X</button>';
    html_itens += '<br />';
    html_itens += '<br />';
    html_itens += '<br />';
    html_itens += '<p class="text" style="margin-bottom: 10px; color: #969696; font-weight: 600;">Nome</p>';
    html_itens += '<input name="name-'+ itens_job +'" type="text" class="form-control required" style="font-size: 14px; background-color: #fff;" >';
    html_itens += '<p class="text margin-t-20" style="margin-bottom: 10px; color: #969696; font-weight: 600;">Valor</p>';
    html_itens += '<input name="value-'+ itens_job +'" min="0" type="number" class="form-control required" style="font-size: 14px; background-color: #fff;" >';
    html_itens += '<p class="text margin-t-20" style="margin-bottom: 10px; color: #969696; font-weight: 600;">Tipo</p>';
    html_itens += '<select class="form-control required" name="type-'+ itens_job +'" style="font-size: 14px; background-color: #fff;">';
    ...
    html_itens += '<div class="input-date">';
    html_itens += '<div class="icon ion-android-calendar arrow"></div>';
    html_itens += '<input type="text" class="form-control add-date required" name="date-'+ itens_job +'" value="'+ dateInput +'" style="cursor: pointer;">';
    html_itens += '<div class="box-datepicker client">';
    html_itens += '<div class="datepicker" data-date="<?= h($extract-date); ?>" data-id="#document-date-'+ itens_job +'"></div>';
    html_itens += '</div>';
    html_itens += '</div>';
    html_itens += '</div>';
